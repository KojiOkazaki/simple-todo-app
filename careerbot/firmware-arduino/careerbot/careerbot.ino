...
void drawScreen(const char* caption = nullptr) {
  ...
  // logo: embedded RGB565 (preferred) -> LittleFS PNG -> simple mark
  int cx = d.width() / 2;
#ifdef HAVE_LOGO
  d.pushImage(cx - logo_w / 2, 20, logo_w, logo_h, logo_data);
#else
  if (LittleFS.exists("/careerbot.png")) {
    d.drawPngFile(LittleFS, "/careerbot.png", cx - 70, 30, 140, 140);
  } else {
    d.fillRoundRect(cx - 60, 40, 120, 120, 16, 0x2C9F);
  }
#endif
